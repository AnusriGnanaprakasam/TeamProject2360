...
public class ViewUserUpdate {
	...
	private static Alert Emailerror = new Alert(AlertType.INFORMATION);
	...
	private ViewUserUpdate() {
        ...
        button_UpdatePassword.setOnAction((_) -> {
            ...
            if (!passwordError.isEmpty()) {
            	...
            	alert.setContentText(passwordError + "\n\n");
                alert.showAndWait();
                return;
            }
        ...
        button_UpdateFirstName.setOnAction((_) -> {result = dialogUpdateFirstName.showAndWait();
	    	...
	    	String nameError = guiTools.NameRecognizer.checkForValidName(newName);
	    	if (!nameError.isEmpty()) {
	            Alert alert = new Alert(Alert.AlertType.ERROR);
	            alert.setContentText(
	                nameError +
	                "\nError at position " +
	                (guiTools.NameRecognizer.nameRecognizerIndexOfError + 1)
	            );
	            alert.showAndWait();
	            return; //do not update DB in this case
	        }
        ...
        button_UpdateEmailAddress.setOnAction((_) -> {result = dialogUpdateEmailAddresss.showAndWait();
    		...
    		String newEmail = theDatabase.getCurrentEmailAddress();
           	...
           	String emailError = guiTools.EmailAddressRecognizer.checkEmailAddress(newEmail);

           	if (!emailError.isEmpty()) {
           	    StringBuilder emailMessage = new StringBuilder();
           	    emailMessage.append(emailError)
           	        .append("\nThere is an error at position ")
           	        .append(guiTools.EmailAddressRecognizer.emailAddressIndexofError + 1)
           	        .append(" in your email address.\n");
           	    // show email alert for updating the email
           	    ViewUserUpdate.Emailerror.setTitle("***ERROR***");
           	    ViewUserUpdate.Emailerror.setContentText(emailMessage.toString());
           	    ViewUserUpdate.Emailerror.setHeaderText("Your email is incorrectly formatted");
           	    ViewUserUpdate.Emailerror.showAndWait();
