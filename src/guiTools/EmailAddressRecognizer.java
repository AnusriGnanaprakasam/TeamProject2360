	...
	public static String checkEmailAddress(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				//Validates the first character of the email
				//Email addresses cannot start with a special character or dot.
				...
				case 1:
					// Stays in state 1 if there is Alphanumeric or special character 
					// If there is a dot it goes to state 0
					// If there is @  it goes to state 2 
					if ((currentChar >= 'A' && currentChar <= 'Z') ||
						(currentChar >= 'a' && currentChar <= 'z') ||
						(currentChar >= '0' && currentChar <= '9') ||
				...
				case 2:
					// validations the start of a domain so what's after @ or after a dot
					if ((currentChar >= 'A' && currentChar <= 'Z') ||
						(currentChar >= 'a' && currentChar <= 'z') ||
						(currentChar >= '0' && currentChar <= '9')) {
				...
				case 3:
					// if the character it gets is alphanumeric it will stay in state 3
					//if we get a dot we go back to state 2 
					// If we get a -  we go to state 4
					finalState = true;
		...
		case 0:
			...
			emailAddressIndexofError = currentCharNdx;		// Copy the index of the current character;
			emailAddressErrorMessage = "May only be alphanumberic for a character starting a domain\n";
			return emailAddressErrorMessage;

		case 1:
			// State 1 is not a final state, so we can return a very specific error message
			emailAddressErrorMessage = "May only be alphanumberic or special characters. Please make sure you have a @ or . character for the email domain\n";
			return emailAddressErrorMessage;
			// Replace this with the required code

		case 2:
			// State 2 is not a final state, so we can return a very specific error message
			emailAddressErrorMessage = "Emails must start with an alphanumberic character.\n";
			return emailAddressErrorMessage;		
			// Replace this with the required code
		...
		case 4: //Domain labels may not end with a hyphen.
			// State 4 is not a final state, so we can return a very specific error message. 
			emailAddressErrorMessage = "email domains may not end with a hyphen.\n";
			return emailAddressErrorMessage;
			// Replace this with the required code
